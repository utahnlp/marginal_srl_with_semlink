...
Modified to work on mixed precision:
    1. added option to specify device
    2. changed log likelihood to neg log
"""
...
class ConditionalRandomField(torch.nn.Module):
    ...
    gpuid : int (default: -1)
        -1 to Force computation to be on cpu for better numerical precision
    """
    ...
    def _input_likelihood(self, logits: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        transitions = to_device(self.transitions, self.gpuid)
        start_transitions = to_device(self.start_transitions, self.gpuid)
        end_transitions = to_device(self.end_transitions, self.gpuid)
    ...
    def _marginal_likelihood(self, logits: torch.Tensor, tag_mask: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        """
        Computes the (batch_size,) nominator term for the log-likelihood, which is the
        sum of the likelihoods across all possible state sequences that are masked by tag_mask.
        """
        batch_size, sequence_length, num_tags = logits.size()

        # Transpose batch size and sequence dimensions
        tag_mask = tag_mask.float().transpose(0, 1).contiguous()
        mask = mask.float().transpose(0, 1).contiguous()
        logits = logits.transpose(0, 1).contiguous()
        ...
        for i in range(1, sequence_length):
            ...
            # Alpha is for the current_tag, so we broadcast along the next_tag axis.
            broadcast_alpha = alpha.view(batch_size, num_tags, 1)
            # Mask out the invalid tag band beam
            cur_tag_mask = tag_mask[i].view(batch_size, 1, num_tags)

            # Add all the scores together and logexp over the current_tag axis
            inner = broadcast_alpha + emit_scores + transition_scores + (1 - cur_tag_mask) * -10000.0
    ...
    def marginal(
        self, inputs: torch.Tensor, tag_mask: torch.ByteTensor, mask: torch.ByteTensor = None
    ) -> torch.Tensor:
        """
        ...
        log_denominator = self._input_likelihood(inputs, mask)
        log_numerator = self._marginal_likelihood(inputs, tag_mask, mask)
    ...
    def viterbi_tags(
        ...
        start_tag = num_tags
        end_tag = num_tags + 1
        _transitions = torch.Tensor(num_tags + 2, num_tags + 2).fill_(-10000.0)
        ...
        constrained_transitions = transitions * self._constraint_mask[
            :num_tags, :num_tags
        ] + -1e8 * (1 - self._constraint_mask[:num_tags, :num_tags])
        _transitions[:num_tags, :num_tags] = constrained_transitions.data
        ...
        if self.include_start_end_transitions:
            ...
            _transitions[:num_tags, end_tag] = end_transitions.detach() * self._constraint_mask[
                :num_tags, end_tag
            ].data + -10000.0 * (1 - self._constraint_mask[:num_tags, end_tag].detach())
        else:
            _transitions[start_tag, :num_tags] = -10000.0 * (
                1 - self._constraint_mask[start_tag, :num_tags].detach()
            )
            _transitions[:num_tags, end_tag] = -10000.0 * (
