...
# encoder for inputs with aligned predicate (WPA)
class BertWPAEncoder(torch.nn.Module):
	def __init__(self, opt, shared):
		super(BertWPAEncoder, self).__init__()
		self.opt = opt
		self.shared = shared
	...
	def forward(self, batch):
		...
		wpa_idx = to_device(batch.wpa_idx, self.opt.gpuid)
		assert(self.shared.seq_l == tok_idx.shape[1])

		max_wpa_l = (wpa_idx != self.pad_token_id).sum(-1).max().long()
		max_concat_l = tok_idx.shape[1] + max_wpa_l
		concated_input = torch.cat([tok_idx, wpa_idx], dim=1)		
